...
Quality categorical varibale follows a fairly normal distribution; therefore, Converting the variable back into a numeric value to cluster. The clusters with the highest average quality score should have the highest portion of high quality wine. We see the characteristics the wine critics are looking for in a good wine.
...
wine_data_numeric %>% 
  select(quality, alcohol, sulphates, volatile.acidity) -> wine_data_numeric
...
plot_ly(wine_data_numeric, 
        x = ~alcohol, 
        y = ~sulphates, 
        z = ~volatile.acidity, 
        color = as.factor(kmeans_result$cluster),  # Add cluster color
        type = "scatter3d", 
        mode = "markers",
        marker = list(size = 3))
